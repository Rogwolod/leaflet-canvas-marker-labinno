...
function layerFactory(L) {
    ...
    var CanvasIconLayer = (L.Layer ? L.Layer : L.Class).extend({
        ...
        removeMarker: function (marker, redraw) {
            if (!marker) {
                console.warn('LeafletCanvas: marker undefined');
                return;
            }
            var self = this;
        ...
        _drawMarker: function (marker, pointPos) {
            ...
            if (marker.canvas_img) {
                self._rotateAndPaintImage(marker, pointPos);
                ...
                if (self._imageLookup[iconUrl]) {
                    ...
                    i.onload = function () {
                        ...
                        self._imageLookup[iconUrl][2].forEach(function (e) {
                            ...
                            self._rotateAndPaintImage(e[0], e[1]);
        ...
        _rotateAndPaintImage(marker, pointPos) {
            var orientationDegree = marker.options.rotationAngle;
            if (!orientationDegree) {
                orientationDegree = 0;
            }
            var options = marker.options.icon.options;
            var angleInRad = orientationDegree * Math.PI / 180;
            ...
            this._context.rotate(angleInRad);
            this._context.drawImage(marker.canvas_img, -width, -height);
            this._context.rotate(-angleInRad);
            this._context.translate(-x, -y);
        },

        _drawImage: function (img, x, y, width, height) {
            console.error("Use _rotateAndPaintImage instead");
        },
